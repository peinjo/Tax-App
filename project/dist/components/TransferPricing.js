    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex space-x-4 border-b border-gray-200 mb-6">
          <button className={`pb-4 px-4 ${activeTab === 'masterFile'
            ? 'border-b-2 border-blue-500 text-blue-600'
            : 'text-gray-500 hover:text-gray-700'}`} onClick={() => setActiveTab('masterFile')}>
            Master File
          </button>
          <button className={`pb-4 px-4 ${activeTab === 'localFile'
            ? 'border-b-2 border-blue-500 text-blue-600'
            : 'text-gray-500 hover:text-gray-700'}`} onClick={() => setActiveTab('localFile')}>
            Local File
          </button>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((item) => (<div key={item} className="p-4 border border-gray-200 rounded-lg hover:border-blue-500 transition-colors">
