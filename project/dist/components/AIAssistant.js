...
export function AIAssistant() {
    ...
    return (<div className="flex flex-col h-[calc(100vh-2rem)] max-w-4xl mx-auto">
      ...
      <div className="flex-1 bg-white rounded-xl shadow-sm p-4 mb-4 overflow-hidden flex flex-col">
        {error && (<div className="flex items-center gap-2 p-3 mb-4 bg-red-50 text-red-700 rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="flex gap-2 pt-4 border-t">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="btn btn-secondary p-2" title="Upload document" disabled={isLoading}>
            <Upload className="h-5 w-5"/>
          </button>
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type your message..." className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={isLoading}/>
          <button type="submit" className="btn btn-primary p-2" disabled={isLoading || !input.trim()}>
