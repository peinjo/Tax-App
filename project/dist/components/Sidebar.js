...
import { BarChart3, FileText, Globe2, Calendar, Bell, Settings, Bot, Target, } from 'lucide-react';
const menuItems = [
...
export function Sidebar({ onClose }) {
    ...
    return (<div className="h-screen w-64 bg-gray-900 text-white p-4">
      ...
      <nav className="space-y-2">
        {menuItems.map((item, index) => (<button key={index} onClick={() => handleNavigation(item.path)} className={`flex items-center gap-3 w-full p-3 rounded-lg transition-colors ${location.pathname === item.path
